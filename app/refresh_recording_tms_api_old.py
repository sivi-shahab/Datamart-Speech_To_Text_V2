...
WITH hist AS (
  ...
  WHERE h.status = 4 AND h.last_response_reason = 'Agree' LIMIT 10
  /**DATE_RANGE**/
),
rec AS (
  SELECT
...
joined AS (
  ...
  FROM hist h
  LEFT JOIN rec r
    ON r.recording_customer_id = h.prospect_id
)
...
SELECT
  ...
  COALESCE((j.created_time)::timestamp, to_timestamp(j.mis_date,'YYYYMMDD')) AS created_time,
  COALESCE((j.created_time)::date,       to_date(j.mis_date,'YYYYMMDD'))      AS created_date,

  j.prospect_id                                           AS recording_customer_id,
  j.last_contact_time, j.a_number, j.context,
